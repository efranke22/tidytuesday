...
Cluster the chocolates using `total_fat_g`, `sodium_mg`, and `total_carbs_g`. On the final plot, we will plot the drinks by `cocoa_percent` and `rating`. 
```{r}
choc_sub <- clean_choc %>%
...
Check out the within-cluster sum of squares. 3 clusters seems reasonable. 
```{r}
